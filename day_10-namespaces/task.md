...
5. Your pod-to-pod connection should work, and you should be able to get a successful response back. (Works always)
...
9. This curl should work. (works)
10. Now try curling the service name instead of IP. You will notice that you are getting an error and cannot resolve the host. (correct)
11. Now use the FQDN of the service and try to curl again, this should work. {cat /etc/resolv.conf - ex. (curl services-name.ns2.svc.cluster.local)}
